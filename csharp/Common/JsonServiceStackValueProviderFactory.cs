...
public class JsonServiceStackValueProviderFactory : ValueProviderFactory
{
    public override IValueProvider GetValueProvider(ControllerContext controllerContext)
    {
        ...
        return new DictionaryValueProvider<object>(ServiceStack.Text.JsonSerializer.DeserializeFromStream<Dictionary<string, object>>(reader).AsExpandoObject(),
...
public class JsonServiceStackResult : JsonResult
{
    public override void ExecuteResult(ControllerContext context)
    {
        ...
        JsConfig<DateTime?>.SerializeFn = date => date != null ? new DateTime(date.Value.Ticks, DateTimeKind.Local).ToString("dd/MM/yyyy HH:mm:ss", Shared.CultureInfoTh) : null;
        ...
        if (Data != null)
        {
            ...
            if (!string.IsNullOrEmpty(callbackfunction))
            {
                ...
                response.Write(string.Format("{0}({1});", callbackfunction, ServiceStack.Text.JsonSerializer.SerializeToString(Data)));
            }
            else
            {
